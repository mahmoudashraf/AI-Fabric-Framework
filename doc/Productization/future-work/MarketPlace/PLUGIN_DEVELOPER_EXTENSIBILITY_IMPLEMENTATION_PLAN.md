...
The first implementation should support three public plugin surfaces plus one internal shell-facing contribution surface.
...
### 3.4 Internal shell contributions

Template, action, and data plugins may also emit internal `ShellContribution` fragments.

These should resolve into deployment-level `shellConfig` and only target fixed platform-owned module, card, and UI block registries.

There should not be a separate public shell-plugin runtime in the first implementation.

### 3.5 Future shell modules
...
- shell behavior must resolve from deployment-level `shellConfig`, not directly from publisher package metadata
...
- arbitrary shell frontend code

### 7.3 Resolution rule

A template plugin should resolve into a normal deployment bootstrap request, which the backend then converts into a normal deployment draft through the same seeding path used by internal presets. Any template-provided shell defaults should resolve into deployment-level `shellConfig`.
...
Resolved action plugins should compile into the existing deployment `actionsConfig` behavior model plus any platform-owned action metadata and `ShellContribution` fragments needed for runtime and shell presentation.
...
Resolved data plugins should compile into deployment-level `knowledgeSourceConfig` or equivalent search-source bindings consumed by the runtime's approved retrieval abstraction, plus optional `ShellContribution` fragments for built-in evidence and search presentation.
...
- no arbitrary publisher shell code
...
- validation pipeline
- catalog persistence
- `ShellContribution` resolution through fixed registries
...
### Phase 6: Billing and revenue share expansion

Reason:

- business layers should not block the core architectural foundation

### Phase 7: Future shell module references through fixed registry
