...
public interface ActionHandler {
    /**
     * Get the action name that this handler manages.
     */
     ...
     * Execute the action.
     */
    ActionResult execute(String userId, Map<String, Object> params);
    
    /**
     * Validate if user is allowed to perform this action.
     */
    boolean validateActionAllowed(String userId);
    
    /**
     * Get confirmation message before execution.
     */
    ...
    public ActionResult execute(String userId, Map<String, Object> params) {
        try {
            ...
            return ActionResult.builder()
                ...
                .data(Map.of("error", "EXECUTION_FAILED"))
                .build();
        }
    }
...
class RelationshipQueryActionHandlerTest {
    ...
    @Test
    void getActionName_shouldReturnRelationshipQuery() {
        assertThat(handler.getActionName()).isEqualTo("relationship_query");
    }
    ...
    void execute_withValidParams_shouldSucceed() {
        ...
        // Act
        ActionResult result = handler.execute("user-123", params);
...
- [ ] **IMPORTANT:** Extract and pass `entityTypes` from intent (avoid redundant LLM calls)
...
4. Orchestrator SHOULD pass entity types (efficiency optimization)
5. Add comprehensive RealAPI integration tests
6. Support both direct and orchestrated patterns

**Important Notes:**
- The relationship query module ALREADY implements sophisticated hybrid vector+relational search
- This work is about: (a) Simplifying direct usage API, (b) Integrating with orchestrator
- Orchestrated pattern is MORE efficient (entity types extracted once, reused)
