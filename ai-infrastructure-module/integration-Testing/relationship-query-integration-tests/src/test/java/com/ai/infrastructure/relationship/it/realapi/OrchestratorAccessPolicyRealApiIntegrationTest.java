...
class OrchestratorAccessPolicyRealApiIntegrationTest {
    ...
    @Test
    @DisplayName("Planner should not add example-leaked brands when user asked for a specific brand")
    void shouldNotAddExampleLeakedBrandValues() {
        OrchestrationContext context = OrchestrationContext.builder()
            .userId("entity-extraction-test-user")
            .sessionId("entity-extraction-session")
            .metadata(Map.of("channel", "test"))
        ...
        OrchestrationResult result = orchestrator.orchestrate(
            "relationship query: show me blue shoes under $100 from Nike",
            context
        );
        ...
        RelationshipQueryPlan plan = extractRelationshipQueryPlanObject(payload);
        assertThat(plan.getPrimaryEntityType()).isEqualTo("product");

        // Assert brand filter contains Nike only (no example leakage like Adidas).
        assertThat(plan.getRelationshipPaths())
            .as("Expected a relationship path to brand")
            .isNotNull();

        boolean hasNike = false;
        boolean hasAdidas = false;

        for (var path : plan.getRelationshipPaths()) {
            if (path == null || path.getConditions() == null) {
                continue;
            }
            for (var cond : path.getConditions()) {
                if (cond == null) {
                    continue;
                }
                String entityType = cond.getEntityType() != null ? cond.getEntityType().toLowerCase() : "";
                String field = cond.getField() != null ? cond.getField().toLowerCase() : "";
                if (!entityType.contains("brand") || !(field.equals("name") || field.endsWith(".name"))) {
                    continue;
                }
                ...
                if (value instanceof List<?> list) {
                    for (Object v : list) {
                        ...
                        hasNike |= s.contains("nike");
                        hasAdidas |= s.contains("adidas");
                    }
                } else if (value != null) {
        ...
        assertThat(hasNike)
            .as("Brand constraint should include Nike")
            .isTrue();
        assertThat(hasAdidas)
            .as("Brand constraint should NOT include Adidas when user only asked for Nike")
            .isFalse();
    }

    @SuppressWarnings("unchecked")
    private RelationshipQueryPlan extractRelationshipQueryPlanObject(Map<String, Object> payload) {
        ...
        throw new AssertionError("Expected plan to be RelationshipQueryPlan but was: " + (planObj != null ? planObj.getClass() : null));
    }
