...
public class RealAPIIntegrationTest {
    ...
    private static final Duration VECTOR_AWAIT_TIMEOUT = Duration.ofSeconds(30);
    ...
    public void testRealOpenAIEmbeddingGeneration() {
        ...
        RealAPITestSupport.awaitVectorExists(vectorManagementService, "test-product", entityId, VECTOR_AWAIT_TIMEOUT);
        VectorRecord entity = vectorManagementService.getVector("test-product", entityId)
            .orElseThrow(() -> new AssertionError("Expected vector to exist for entity test-product:" + entityId));
        ...
        assertTrue(entity.getContent().length() > 100, "Should have substantial searchable content");
        assertTrue(entity.getContent().contains("AI-Powered"), "Should contain product name");
        assertTrue(entity.getContent().contains("artificial intelligence"), "Should contain AI-related content");
        assertTrue(entity.getContent().contains("smart home"), "Should contain category content");
    ...
    public void testRealAIContentAnalysis() {
        ...
        List<VectorRecord> allEntities = products.stream()
            .map(product -> {
                String entityId = product.getId().toString();
                RealAPITestSupport.awaitVectorExists(vectorManagementService, "test-product", entityId, VECTOR_AWAIT_TIMEOUT);
                return vectorManagementService.getVector("test-product", entityId)
                    .orElseThrow(() -> new AssertionError("Expected vector to exist for entity test-product:" + entityId));
            })
        ...
        List<VectorRecord> aiResults = filterByContent(allEntities, "artificial intelligence");
        assertTrue(aiResults.size() >= 1, "Should find AI-related content");

        // Search for machine learning content
        List<VectorRecord> mlResults = filterByContent(allEntities, "machine learning");
        assertTrue(mlResults.size() >= 1, "Should find machine learning content");

        // Search for analytics content
        List<VectorRecord> analyticsResults = filterByContent(allEntities, "analytics");
        ...
        for (VectorRecord entity : allEntities) {
            ...
            assertTrue(entity.getContent().length() > 50, "Each entity should have substantial content");
        }
    ...
    public void testRealAISemanticSearch() {
        ...
        List<VectorRecord> audioResults = filterByContent(allEntities, "audio");
        ...
        List<VectorRecord> wirelessResults = filterByContent(allEntities, "wireless");
        ...
        List<VectorRecord> gamingResults = filterByContent(allEntities, "gaming");
    ...
    public void testRealAIEndToEndWorkflow() {
        ...
        List<VectorRecord> allEntities = vectorManagementService.getVectorsByEntityType("test-product");
        ...
        List<VectorRecord> aiResults = filterByContent(allEntities, "AI");
        assertTrue(aiResults.size() >= 2, "Should find AI-related products");

        List<VectorRecord> smartResults = filterByContent(allEntities, "smart");
        assertTrue(smartResults.size() >= 2, "Should find smart products");

        List<VectorRecord> securityResults = filterByContent(allEntities, "security");
    ...
    private List<VectorRecord> filterByContent(List<VectorRecord> source, String term) {
