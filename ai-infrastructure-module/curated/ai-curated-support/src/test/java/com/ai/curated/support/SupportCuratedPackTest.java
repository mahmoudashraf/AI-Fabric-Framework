...
class SupportCuratedPackTest {
    ...
    void shouldLoadSupportPackDefaultsAndTemplates() {
        ...
        assertThat(promptBundle.getOverlays()).contains("v1-support");

        PromptTemplateResolver resolver = new PromptTemplateResolver(
            new ClasspathPromptTemplateStore(new DefaultResourceLoader()),
            promptBundle
        );
        assertThat(resolver.resolve("intent-extraction/multi-step", "classify").template().key().version()).isEqualTo("v1-support");
    }
}
