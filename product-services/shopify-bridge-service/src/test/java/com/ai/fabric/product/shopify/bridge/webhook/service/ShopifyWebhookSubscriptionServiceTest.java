...
class ShopifyWebhookSubscriptionServiceTest {
    ...
    void reconcileCreatesMissingSubscriptions() {
        ...
        verify(client, times(13)).execute(eq("alpha.myshopify.com"), eq("token"), eq(listQuery()), anyMap());
        verify(client, times(13)).execute(eq("alpha.myshopify.com"), eq("token"), eq(createMutation()), anyMap());
    }
    ...
    void inspectMarksMissingAndDriftedTopics() {
        ...
        assertThat(summary.expectedCount()).isEqualTo(13);
        assertThat(summary.readyCount()).isEqualTo(1);
        assertThat(summary.missingCount()).isEqualTo(11);
