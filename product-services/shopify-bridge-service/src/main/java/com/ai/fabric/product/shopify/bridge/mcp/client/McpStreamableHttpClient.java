...
public class McpStreamableHttpClient {
    ...
    public McpStreamableHttpClient(RestClient.Builder restClientBuilder, ObjectMapper objectMapper) {
        this.restClient = restClientBuilder.build();
        this.objectMapper = objectMapper;
    }

    public McpSession initialize(URI endpoint, McpRequestOptions options) {
        McpRequestOptions resolvedOptions = options == null ? McpRequestOptions.none() : options;
    ...
    public JsonNode toolsList(URI endpoint, McpRequestOptions options) {
        return toolsList(new McpSession(
            endpoint,
            options == null ? DEFAULT_PROTOCOL_VERSION : options.protocolVersion(),
            null,
            null
        ), options);
    }
    ...
    public JsonNode toolsCall(URI endpoint, String toolName, JsonNode arguments, McpRequestOptions options) {
        return toolsCall(new McpSession(
            endpoint,
            options == null ? DEFAULT_PROTOCOL_VERSION : options.protocolVersion(),
            null,
            null
        ), toolName, arguments, options);
    }
    ...
    public record McpRequestOptions(String protocolVersion, Map<String, String> headers) {
        public McpRequestOptions {
            protocolVersion = StringUtils.hasText(protocolVersion) ? protocolVersion.trim() : DEFAULT_PROTOCOL_VERSION;
            headers = headers == null ? Map.of() : Map.copyOf(headers);
        }

        public static McpRequestOptions none() {
            return new McpRequestOptions(DEFAULT_PROTOCOL_VERSION, Map.of());
        }

        public static McpRequestOptions withHeaders(Map<String, String> headers) {
            return new McpRequestOptions(DEFAULT_PROTOCOL_VERSION, headers);
        }
    }
