# Ecommerce Store (Domain API + REST Connector + Runtime)
...
OPENAI_ENABLED=true \
OPENAI_API_KEY="$(tr -d '\n' < scripts/openai.env)" \
docker compose -f Real_Apps/ecommerce-store/deploy/docker/docker-compose.yml up --build
```
...
- Domain API (this app): `Real_Apps/ecommerce-store/deploy/railway/RAILWAY_DEPLOYMENT_GUIDE.md`
