...
if [[ "${RUN_PLATFORM_ADMIN_CHECKS}" == "true" && -z "${PLATFORM_UI_BASE_URL}" ]]; then
  echo "Set PLATFORM_UI_BASE_URL when platform admin checks are enabled." >&2
  exit 2
fi
...
if [[ ("${RUN_PLATFORM_ADMIN_CHECKS}" == "true" || "${RUN_ECOMMERCE_DEPLOYMENT_CHECKS}" == "true" || "${RUN_MARKETPLACE_RUNTIME_CHECKS}" == "true" || "${RUN_VECTOR_DEPLOYMENT_CHECKS}" == "true" || "${RUN_MARKETPLACE_INSTALL_FLOW_CHECKS}" == "true") && -z "${PLATFORM_API_KEY}" && ( -z "${PLATFORM_LOGIN_EMAIL}" || -z "${PLATFORM_LOGIN_PASSWORD}" ) ]]; then
...
if [[ "${RUN_PLATFORM_ADMIN_CHECKS}" == "true" ]]; then
  admin_target_deployment_id="${ADMIN_TARGET_DEPLOYMENT_ID:-${ECOMMERCE_DEPLOYMENT_ID:-${QDRANT_DEPLOYMENT_ID}}}"
  ...
  run_step "Verify platform admin live regression" env \
    ...
    VERIFY_DEPLOYMENT_OVERRIDE_SMOKE="true" \
    VERIFY_CANONICAL_ROLLOUT_READONLY="true" \
    VERIFY_CANONICAL_ROLLOUT_MUTATION="false" \
