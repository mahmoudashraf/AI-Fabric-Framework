...
refresh_platform_release_verification_evidence_if_needed() {
  ...
  while true; do
    ...
    should_recheck="$(PARSE_BODY="${HTTP_BODY}" EXPECT_RELEASE_ID="${PLATFORM_EXPECT_RELEASE_ID}" EXPECT_VERSION_ID="${PLATFORM_EXPECT_VERSION_ID}" EXPECT_STATUS="${PLATFORM_EXPECT_VERIFICATION_STATUS}" python3 - <<'PY'
import json
import os
