...
| `ONBOARDING_VECTORIZATION_LAYER_PLAN.md` | New active Track B plan | Reframes Track B around onboarding-time indexing into deployment-configured entities and selected/provisioned vector databases, with deployment-scoped ephemeral runners, bootstrap indexing when vectors are absent, customer-selected reindex on relevant config changes, and coarse tracking | Actual implementation of Track B items `56` through `60` |
| `VECTORIZATION_LAYER_CODE_RESIDENCY_AND_INTEGRATION_PLAN.md` | New active Track B companion plan | Establishes the concrete code-residency model for platform vectorization control plane, product-shared integration primitives, product vectorization core, per-deployment runner provisioning, bootstrap detection, reindex decision flow, and pull-only runner control flow | Actual implementation of Track B items `56` through `60` |
