...
- Remaining live operation after deploy: store a fresh `PARTNER_SUPABASE_JWT` platform secret, dispatch `full-platform-release-readiness`, confirm the Partner Enablement stage passes in strict mode, and confirm `/api/verification-suites/release-gate` reports `READY`.
