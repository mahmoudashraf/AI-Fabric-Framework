...
- Deployment note: first Coolify staging deploy failed because Dockerfile health checks require `curl` or `wget` inside the image. Commit `99056be7a` adds `curl` to the image, and the follow-up Coolify deployment `oin8ayq4cukruir95hrvu661` reached `running:healthy`.
...
- Hosted verification passed: `/health` returned `UP`, merchant and partner pages served claim-safe copy over HTTPS, `/api/leads` accepted a synthetic staging smoke request, and Playwright browser checks passed for hosted desktop/mobile pages. Hosted screenshots were written to `/tmp/loomai-landing-site-hosted`.
