...
- Current release status: production infrastructure can deploy `Platform-V10`, and Gates 4/5 are complete for controlled proof readiness. Public/prod launch is still blocked until Gate 6 controlled Platform `Go production` mutation proof, Gate 9 rollback/deactivation proof, failed-promotion staging-isolation proof, production DNS posture, Coolify health-check posture, and final support/App Store packaging are complete.
