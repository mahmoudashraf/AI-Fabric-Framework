...
- Shopify [redacted platform login password] UI refactor target: merchant-first workflows, with platform/operator diagnostics hidden in Advanced.
- Shopify [redacted platform login password] UI changed in `product-services/shopify-bridge-service/ui/src/App.tsx`: added tabs Home/Setup/Insights/Billing/Support/Go live/Advanced, simplified merchant summary, moved raw bundles/dossiers/webhook/vectorization internals to Advanced, added Billing section.
- Verification for Shopify [redacted platform login password] UI refactor: `npm --prefix product-services/shopify-bridge-service/ui run build` passed; targeted Shopify backend tests passed; `git diff --check` passed; live bridge shell smoke passed.
- Local Shopify [redacted platform login password] UI dev server was started on `http://127.0.0.1:5175/`.
- Production Shopify bridge root served a bundle containing new `Loom Companion` [redacted platform login password] UI strings after commit `277da57d`; merchant may need Shopify Admin/hard refresh to see it.
...
- Created `doc/Productization/future-work/MarketPlace/Products/Strategy/PARTNER_DASHBOARD_STRATEGY_PLAN.md` and updated `Codex_Strategic_Context.md`: partner dashboard owns runbooks/packets/diagnostics/referrals/commissions; merchant [redacted platform login password] stays merchant-safe.
...
- Documented Shopify bridge [redacted platform login password] verification key rule: `SHOPIFY_BRIDGE_ADMIN_API_KEY` must match deployed `SHOPIFY_BRIDGE_SHARED_SECRET`, uses `X-BRIDGE-API-KEY` by default, and is distinct from `SHOPIFY_ADMIN_ACCESS_TOKEN`.
...
- Storefront Product Shell accepted as complete: hosted Shopify extension deploy complete, browser proof complete, bridge [redacted platform login password] verification complete, full live verifier passed with [redacted platform login password] checks enabled, docs/context committed and pushed through `e50e46d2`. No pending handoff items reported.
...
- Live verification passed against production Platform/Bridge for `shopping-companion-test.myshopify.com` with [redacted platform login password] checks skipped: `scripts/verify-shopify-companion.sh` passed; bootstrap showed Free=`ai-search`, modes=`navigator`, `chatFallbackEnabled=false`, and no governed-action capability; nested and legacy top-level `comparison` direct-route requests both returned HTTP `403`.
- Blockers: the available local bridge [redacted platform login password] key returned HTTP `401` from `/api/[redacted platform login password]/overview`, so [redacted platform login password] checks need the deployed `SHOPIFY_BRIDGE_SHARED_SECRET` value; `shopify:app:deploy` is blocked by Shopify CLI interactive login/no non-interactive token, so browser-level proof that the Shopify-hosted theme extension opens Max Mode from embedded surfaces remains pending.
- Next handoff: provide or configure the deployed bridge shared secret for [redacted platform login password] live checks, complete authenticated Shopify app/theme extension deploy, then browser-verify embedded surface to Max Mode page-context and attachment handoff on desktop/mobile.
...
- Local secret/config search found no usable non-interactive secret source beyond the existing local bridge [redacted platform login password] key file; `railway` and `gh` CLIs are unavailable; Shopify CLI is available at version `3.93.2`.
- `npm --prefix product-services/shopify-bridge-service run shopify:preflight` passed.
- Bridge [redacted platform login password] retry against production `/api/[redacted platform login password]/overview` with the existing local key file returned HTTP `401`; this still means the local value does not match deployed `SHOPIFY_BRIDGE_SHARED_SECRET`.
...
- Next handoff: export the deployed bridge shared secret as `SHOPIFY_BRIDGE_ADMIN_API_KEY`, export a valid `SHOPIFY_CLI_PARTNERS_TOKEN`, then rerun [redacted platform login password] verification, `shopify:app:info`, `shopify:app:deploy`, and browser proof.
...
- Post-deploy non-[redacted platform login password] `scripts/verify-shopify-companion.sh` passed again for `shopping-companion-test.myshopify.com`.
- Remaining blocker: direct bridge [redacted platform login password] `/api/[redacted platform login password]/overview` still needs the deployed Railway `SHOPIFY_BRIDGE_SHARED_SECRET` exported as `SHOPIFY_BRIDGE_ADMIN_API_KEY`; the private handoff does not contain it and the existing local temp key returns HTTP `401`.
...
- Extracted deployed `SHOPIFY_BRIDGE_SHARED_SECRET` into `/tmp/shopify_bridge_[redacted platform login password]_api_key_from_railway.secret` without printing or committing the value.
- Direct bridge [redacted platform login password] `/api/[redacted platform login password]/overview` returned HTTP `200` with `serviceRef=shopify-bridge-prod`, `status=READY`, `stores=2`, `billingMode=FREE`.
- Full `scripts/verify-shopify-companion.sh` passed with `SHOPIFY_BRIDGE_ADMIN_API_KEY_FILE=/tmp/shopify_bridge_[redacted platform login password]_api_key_from_railway.secret`, including bridge [redacted platform login password] overview, billing, webhook diagnostics, support readiness, usage, vectorization, governed actions, and vectorization source page.
- Storefront Product Shell status: fully live verified; no remaining live verification blocker is known.
- Storefront Product Shell final acceptance update: implementation complete, Shopify hosted extension deploy complete, browser proof complete, bridge [redacted platform login password] verification complete, full live verifier passed with [redacted platform login password] checks enabled, docs/context committed and pushed through `e50e46d2`; no pending handoff items.
...
- Live verification passed after Railway served `/assets/index-C_82foy0.js`: deployed bundle contained `Future Elite demand signals`, `Action-intent questions`, and `Starter remains read-only`; full `scripts/verify-shopify-companion.sh` passed for `shopping-companion-test.myshopify.com` with bridge [redacted platform login password] checks enabled; direct [redacted platform login password] `usage-summary` contained `topQuestionsLast7Days`, `unansweredQuestionsLast7Days`, `actionIntentQuestionsLast7Days`, and `roiSummary.status=EARLY_SIGNAL`.
- Blockers: none.
- Next handoff: no pending 003 items.
- Independent 003 verification pass confirmed commits `64f7093c` and `f40036ab`: `git diff --check`, `bash -n scripts/verify-shopify-companion.sh`, Shopify [redacted platform login password] UI build, targeted 003 Maven suite, full Shopify Bridge Maven suite, deployed bundle proof strings, full live Shopify verifier with bridge [redacted platform login password] checks, and direct [redacted platform login password] `usage-summary` proof all passed. Only unrelated `.DS_Store` remains dirty.
...
- Clarified Partner Enablement authority boundary: [redacted platform login password]s/operators own deployment-level controls; partners/integrators own product implementation workflows; merchants own store-level consent/configuration.
...
- Readiness plan requires compact evidence under `/tmp/shopify-first-product-readiness-audit/`, product-truth scan, entitlement/build/test gates, storefront desktop/mobile proof, merchant [redacted platform login password] proof, live verifier with optional bridge [redacted platform login password] checks, support collateral review, and a final appended completion section.
...
- Added platform/operator Product Readiness Audit UI requirement to 005: overview, checklist, query pack, answer results, evidence, decision panel, stale evidence warning, append-only decision history, and no merchant-[redacted platform login password]/partner-first placement. `DESIGN_PARTNER_READY` requires a reviewable UI/evidence surface, not private scripts or chat history alone.
...
- Decision: Coolify is a first-class deployment provider beside Railway for tenant runtimes and restartable services; Platform UI/backend/Postgres/partner UI/Shopify bridge, billing, webhooks, readiness audit, and provider [redacted platform login password]istration stay on Railway.
...
- Used ignored private handoff section `7.5 Subabse Partner UI` as the Supabase credential source; extracted runtime/[redacted platform login password] values into `/tmp/partner_supabase.env` without committing secrets.
...
- Live verification passed after push: full `scripts/verify-shopify-companion.sh` passed with bridge [redacted platform login password] checks; deployed Bridge UI asset contains `partner-access/requests` and `Partners`; Platform login and `GET /api/merchant/partner-access/requests?shopDomain={shop}` returned HTTP `200`.
- Full live installed-store denial path passed with a temporary confirmed Supabase email partner: signup returned HTTP `200`, eligible-store lookup returned one installed store, implementation creation returned HTTP `201` and `WAITING_ON_MERCHANT`, merchant/[redacted platform login password] listing found the request, denial returned HTTP `200` and `DENIED`, partner final implementation fetch returned `DENIED`, and assigned-store count remained `0`.
...
- Backend changes: added canonical Shopify source-settings update service/[redacted platform login password] endpoint; added partner product-control summaries and partner endpoints for widget settings, source settings, and support profile; enforced provisioned partner context, active assignment, explicit assignment capabilities, installed-store status for writes, and partner-safe response projection with no secrets/runtime/deployment internals.
...
- Verification script changes: strict live verifier now reaches product controls, performs and restores a real partner-authenticated support-profile write, proves canonical Platform/[redacted platform login password] visibility, checks partner-visible product audit activity, and confirms revoked assignments lose product-control access.
...
- Activated `shopping-companion-test.myshopify.com` to `ELITE/ACTIVE` through the bridge [redacted platform login password] billing-state endpoint using the deployed Railway bridge shared-secret value from local private secret files.
...
- Verification passed: targeted Shopify Bridge Maven tests, `git diff --check`, verifier shell syntax checks, full `scripts/verify-shopify-companion.sh` with [redacted platform login password] checks and correct go-live blocker expectations, and `scripts/verify-shopify-companion-max-widget-live.sh` with browser proof.
...
- Generated Coolify root users for both instances through SSH tunnels; credentials are stored only in `/tmp/coolify_[redacted platform login password]_credentials.env` mode `0600`. Do not paste or commit this file.
...
- Final hardening commits pushed to `Platform-V6`: `07d972aa` marketplace grounded smoke query, `ca40b559` Shopify readiness billing posture and Bridge [redacted platform login password] secret registry support, `4ebb4996` no ephemeral Qdrant DB key creation in release gate, `905b0dd7` live hosted probes authoritative over stale persisted evidence.
...
- Live verification prerequisites refreshed without logging secrets: Qdrant data-plane key verified, Shopify Bridge [redacted platform login password] key stored as Platform secret `SHOPIFY_BRIDGE_ADMIN_API_KEY`, and a fresh non-social Supabase partner JWT stored as `PARTNER_SUPABASE_JWT` before the final run.
...
- Reactivated `shopping-companion-test.myshopify.com` to `ELITE/ACTIVE` through the live Shopify Bridge [redacted platform login password] billing-state endpoint, which writes the Platform-recorded Shopify billing state. Secret source was the local private bridge [redacted platform login password] key file; no secret values were printed or committed.
...
- Coolify API enabled on both hosts; generated root credentials remain only in `/tmp/coolify_[redacted platform login password]_credentials.env`, and generated API tokens remain only in `/tmp/coolify_api_tokens.env` mode `0600`. No token values should be pasted into docs or chat.
...
- Continued from the strict staging app smoke failure on `Platform-V8`; no secrets were intentionally printed or committed. Local secret files remain `/tmp/hetzner_cloud_token.secret`, `/tmp/coolify_api_tokens.env`, and `/tmp/coolify_[redacted platform login password]_credentials.env`.
...
- Added [redacted platform login password]-only `PATCH /api/deployment-provider/target-profiles/{targetProfileId}` with `PatchDeploymentTargetProfileRequest` so operators can activate Coolify target profiles and optionally mark them default without direct DB edits.
...
- Continued after the Coolify runtime reached `running:healthy` but Platform deep verification failed on runtime/[redacted platform login password] and connector/[redacted platform login password] probes.
...
- Backend changes in progress: `CoolifyDeploymentProvider` now resolves Platform secret placeholders through `PlatformSecretService` before sending env vars to Coolify, marks resolved secret env vars as shown-once, creates/updates a paired public-Git REST connector app for Git-source profiles, rewires runtime `ACTIONS_CONNECTOR_BASE_URL` to the connector FQDN, rewires connector `REST_CONNECTOR_RUNTIME_PROXY_BASE_URL` to the runtime FQDN, starts/waits for connector and runtime, and records separate resource handles (`APPLICATION`, `CONNECTOR_APPLICATION`).
- Cleanup change in progress: hard delete now deletes tracked provider resource handles through the provider registry before Platform records are removed; Coolify delete treats 404 as already absent.
...
- Next handoff: update `007` roadmap, run secret scan, commit/push, wait for Railway Platform deployment, then run a disposable staging Platform apply. Expected live result: runtime and connector Coolify apps both reach running/healthy, release verification reaches `APPLIED_VERIFIED`, and Platform hard delete returns staging app count to `0` without direct Coolify cleanup.
